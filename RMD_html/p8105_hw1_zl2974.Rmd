...
In this dataset, we've seen a `r nrow(penguins)`  *  `r ncol(penguins)` size, in which contains column's variables eg. _`r names(penguins)`_.In the __`r names(penguins)[[5]]`__ we are going to discuss, we see that this variable is a `r class(pull(penguins,'flipper_length_mm'))` type variable(`r tail(pull(penguins,'flipper_length_mm'))`). Also we need to see __`r names(penguins)[[1]]`__, which is a `r class(pull(penguins,'species'))` type data, with level of _`r as.character(unique(pull(penguins,'species')))`_.
...
```{r scatterplot,warning=FALSE, echo = F, message= F}
plt_data = penguins %>%
  filter(
...
filename = paste(getwd(),"p8105_hw1_zl2974_flipper_v_bill.png",sep="/")
)
show(plt_flivbil)
```
